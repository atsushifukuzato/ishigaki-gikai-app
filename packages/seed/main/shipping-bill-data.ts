...
const opinionPatterns: Array<{
    ...
    opinions: [
      {
        ...
        content:
          "紙のB/Lは物理的な原本管理が可能だが、電子化された場合のサイバー攻撃や改ざんリスクへの対策が議案では十分に示されていない。ブロックチェーン等の技術的担保と、万一の不正発生時の法的責任の明確化が必要。",
        source_message_content:
          "紙なら原本の管理ができるんですけど、電子化するとサイバー攻撃とか改ざんのリスクが怖いですね。議案にはそのあたりの対策が具体的に書かれてないのが気になります。ブロックチェーンとかで担保するにしても、万一の時の法的責任をはっきりさせてほしい。",
      },
      {
  ...
  // パターン4: 港湾労働者（反対・雇用懸念）
  {
    ...
    opinions: [
      ...
      {
        title: "現場の声が議案に反映されていない",
        content:
          "議案の策定過程で、実際に港湾で働いている我々の意見が十分に聴取されていない。机上の理想論だけで進めてほしくない。現場実態に即した制度設計を求める。",
        source_message_content:
          "議案作る時に、実際に港で働いてる人間の話をちゃんと聞いたんですかね。机の上で考えただけの理想論じゃ困る。現場の実態に合った制度にしてほしいです。",
      },
    ],
  },
  // パターン5: 国際法学者（賛成・制度整備重視）
  {
    ...
    opinions: [
      ...
      },
      {
        ...
        content:
          "紙のB/Lでは裏書・交付による権利移転が確立しているが、電子化においてはこれに相当する確実な権利移転の仕組みを法律で担保する必要がある。本議案の権利移転規定は概ね妥当だが、占有の概念をさらに明確化すべき。",
        source_message_content:
          "紙のB/Lだと裏書で権利移転できるわけですが、電子化するとそれに代わる確実な仕組みが必要です。本議案の権利移転規定はおおむね妥当ですが、占有の概念をもう少し明確にしたほうがいいですね。",
      },
      {
        title: "既存の国際条約との整合性確保",
        content:
          "ヘーグ・ヴィスビー・ルールズやロッテルダム・ルールズとの整合性は議案の現行の枠組みで概ね確保されている。ただし、米国のCOGSA体系を持つ国との取引については、追加的な解釈指針が必要になる場面も想定される。",
        source_message_content:
...
/**
 * 船荷証券議案のインタビューセッションを作成（10パターン × 10ループ = 100件）
 */
...
export function createShippingBillMessages(
  ...
  for (let i = 0; i < sessionIds.length; i++) {
    ...
    messages.push({
      ...
      role: "assistant",
      content:
        "こんにちは！船荷証券の電子化に関する法律案についてお話を伺います。まず、この議案に賛成ですか？反対ですか？",
    });
    messages.push({
      interview_session_id: sessionId,
      role: "user",
...
/**
 * 船荷証券議案のインタビューレポートを作成（100件、各3 opinions）
 */
