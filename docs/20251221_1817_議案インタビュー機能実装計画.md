...
議案インタビュー機能を実装します。AIが事前定義された質問を会話の流れに応じて適切なタイミングで投げかけ、ユーザーの回答を収集します。インタビュー終了時にレポートを生成し、チャット画面内で表示してユーザーの同意を得ます。レポート提案は既存の `/api/interview/chat` を summary 用メタデータ付きで呼び分けて行い、DB保存は `/api/interview/complete` のみで実施します（summaryフェーズでは保存しない）。
...
- [DB設計ドキュメント](docs/20251217_1831_議案インタビュー機能DB設計.md)
